...
import { MicIcon, StopIcon, UploadIcon, CloseIcon, FileTextIcon } from './Icons';
...
function Recorder({ onRecordingComplete, isRecording, setIsRecording, onNotification, hidden, onOpenSettings }) {
  ...
  useEffect(() => {
    ...
    const loadAIConfig = async () => {
      ...
      try {
        ...
        if (!cancelled && request === requestId && response.ok && data.success) {
          setAiConfig(data);
          setApiKeyWarning(null);
        }
      } catch (_) {
        // An unavailable profile is not evidence that a personal key is missing.
      }
    };
    loadAIConfig();
    window.addEventListener('minutes-ai-config-updated', loadAIConfig);
    return () => {
      cancelled = true;
      window.removeEventListener('minutes-ai-config-updated', loadAIConfig);
    };
  }, []);
  ...
  const handleStartRecording = () => {
    const manorManaged = (IS_CLOUD && !aiConfig) || aiConfig?.stt_managed_by === 'manor'
      || aiConfig?.llm_managed_by === 'manor';
    // The server knows about account and deployment keys. Browser flags only
    // describe past manual settings, and are not a readiness check.
    if (!manorManaged && STT_MODE !== 'local' && aiConfig?.stt_configured === false) {
      setApiKeyWarning(LLM_MODE !== 'local' && aiConfig?.llm_configured === false ? 'both' : 'stt');
      return;
    }
    setApiKeyWarning(null);
    startRecording();
  ...
  const startLiveTranscription = (stream) => {
    try {
      ...
      ws.onclose = (event) => setLiveStatus(previous => {
        if (previous.startsWith('Error:')) return previous;
        return event?.code === 4001 ? 'Error: Please sign in again to use live transcription.' : '';
      });
  ...
  return (
    <div className="recorder quiet-recorder" style={hidden ? { display: 'none' } : undefined}>
      <div
        className="recorder-card"
        role="region"
        aria-label={mode === 'upload' ? 'Upload a recording' : 'Record a meeting'}
      >
                ...
                <div className="recorder-form">
                  ...
                  <label className="recorder-toggle">
                    ...
                    <span className="recorder-toggle-text">
                      <span className="recorder-toggle-title">Include meeting audio</span>
                      {captureSystemAudio && (
                        <span className="recorder-toggle-hint">Choose your meeting tab or screen and enable “Share audio”.</span>
                      )}
                    </span>
                  </label>
                  <details className="quiet-disclosure recording-options">
                    <summary>Recording options</summary>
                    <div className="quiet-disclosure-body">
                      {templates.length > 0 && (
        ...
        {mode === 'upload' && !isRecording && (
          <div className="recorder-body">
            <div
              ...
              role="button"
              tabIndex={0}
              aria-label={uploadFile ? `Selected file: ${uploadFile.name}` : 'Choose an audio or video file'}
            ...
            >
              ...
              {uploadFile ? (
                <div className="upload-file-info">
                  ...
                  <button className="file-remove" aria-label="Remove selected file" onClick={(e) => { e.stopPropagation(); setUploadFile(null); }}>
                    <CloseIcon size={16} />
                  </button>
                </div>
              ) : (
                <div className="upload-placeholder">
                  <span className="upload-cta">Drop a file or browse</span>
                  <span className="upload-formats">Audio or video · Up to 500 MB</span>
                </div>
              )}
            </div>
