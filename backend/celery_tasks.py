...
def process_meeting_task(self, meeting_id: str, audio_filepath: str, language: str = None):
    ...
    try:
        ...
        from api.services.llm_config import resolve_processing_llm_config, get_openrouter_client
        manor_managed = classify({"auth_source": meeting.auth_source}) == "manor"
        user_config = {}
        owner_id = meeting.created_by_user_id
        if not manor_managed and owner_id:
            try:
                ...
                try:
                    with user_conn.cursor() as cur:
                        cur.execute(
                            "SELECT stt_api_key, stt_base_url, llm_api_key, llm_base_url, llm_model "
                            "FROM users WHERE id::text = %s OR email = %s LIMIT 1",
                            (owner_id, owner_id),
                        )
                ...
                logger.warning("Could not load meeting owner's AI configuration ({})", type(exc).__name__)

        stt_config = resolve_stt_config(manor_managed=manor_managed, user_config=user_config)
        llm_config = resolve_processing_llm_config(manor_managed=manor_managed, user_config=user_config)
        stt_mode = "local" if stt_config["use_local"] else "cloud"
        llm_mode = "local" if llm_config["use_local"] else "cloud"
        if not stt_config["use_local"] and not stt_config["api_key"]:
            raise RuntimeError(
        ...
        if not llm_config["use_local"] and not llm_config["api_key"]:
            raise RuntimeError(
                "Manor meeting AI is not configured. Please contact support."
                if manor_managed else "No LLM API key configured. Add one in Settings."
            )

        # Each meeting owns its client configuration. Never overwrite the worker
        # environment, which is shared with subsequent meetings and accounts.
        transcription_service = None if stt_config["use_local"] else TranscriptionService(stt_config=stt_config)
        summarization_service = None if llm_config["use_local"] else SummarizationService(
            client=get_openrouter_client(api_key=llm_config["api_key"], base_url=llm_config["base_url"]),
            model=llm_config["model"],
        )
        logger.info(f"Starting processing for meeting {meeting_id} (STT={stt_mode}, LLM={llm_mode})")
