...
async def upload_meeting_audio(
    ...
    try:
        ...
        meeting_auth_source = "manor" if classify(user) == "manor" else "local"
        ...
        # Queue processing after the response is sent.  dispatch_processing
        # enqueues first and only claims PROCESSING once the broker has taken
        # the task, so a broker outage leaves the meeting FAILED with a visible
        # reason instead of stuck in PROCESSING forever.
        def start_processing():
            import time
            from api.services.processing_dispatch import dispatch_processing, EnqueueFailed
            ...
            try:
                dispatch_processing(
                    meeting_id,
                    str(filepath),
                    language=metadata_dict.get("language") or None,
                )
            except EnqueueFailed as e:
                logger.error(
                    f"Meeting {meeting_id} marked failed — could not queue processing: {e}"
                )
    ...
    async def stream_response():
        try:
            ...
            if is_credit_exhausted_error(e):
                yield f"data: {json.dumps({'error': '额度不足,请充值后再试。'})}\n\n"
            else:
...
async def retry_processing(meeting_id: str, background_tasks: BackgroundTasks, user: dict = Depends(get_authenticated_user)):
    ...
    try:
        ...
        # Confirm the meeting is really ours before touching the queue.
        db = get_db_session()
        try:
            db_meeting = db.query(MeetingModel).filter(
                MeetingModel.id == meeting_id,
        ...
        # Enqueue synchronously (.delay() is just a broker publish) so the
        # response reflects what actually happened.  Enqueuing in a background
        # task meant this endpoint reported "restarted successfully" even when
        # the task was never queued — the user then watched it sit in
        # PROCESSING and pressed Retry again, forever.
        from api.services.processing_dispatch import dispatch_processing, EnqueueFailed
        try:
            task_id = dispatch_processing(meeting_id, meeting.audio_file)
        except EnqueueFailed as e:
            logger.error(f"Retry failed for meeting {meeting_id}: {e}")
            raise HTTPException(
                status_code=503,
                detail="Processing queue is unavailable. Please try again in a moment.",
            )
...
async def get_meetings(limit: int = 20, offset: int = 0, status: Optional[str] = None, entity_id: Optional[str] = None, q: Optional[str] = None, sort: Optional[str] = "newest", favorite: Optional[bool] = None, tag: Optional[str] = None, count_total: bool = False):
    ...
    try:
        ...
        if entity_id is not None:
            query = query.filter(MeetingModel.entity_id == str(entity_id))
        else:
