...
from api.config.edition import IS_COMMUNITY
...
async def get_me(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    manor_managed = classify(payload) == "manor"
    user = get_user_by_email(payload.get("email", ""))
    if not user:
        if not manor_managed:
            raise HTTPException(status_code=401, detail="User not found")
        user = payload
    stt = resolve_stt_config(manor_managed=manor_managed, user_config=user)
    llm = resolve_processing_llm_config(manor_managed=manor_managed, user_config=user)
    return JSONResponse({
        "success": True,
        "email": user.get("email", ""),
        "name": user.get("name", ""),
        "stt_managed_by": "manor" if manor_managed else "local",
        "stt_configured": bool(stt["use_local"] or stt["api_key"]),
        "has_stt_key": False if manor_managed else bool(user.get("stt_api_key")),
        "stt_base_url": "" if manor_managed else user.get("stt_base_url") or "",
        "llm_managed_by": "manor" if manor_managed else "local",
        "llm_configured": bool(llm["use_local"] or llm["api_key"]),
        "has_llm_key": False if manor_managed else bool(user.get("llm_api_key")),
        "llm_model": "" if manor_managed else user.get("llm_model") or "",
        "llm_base_url": "" if manor_managed else user.get("llm_base_url") or "",
...
async def update_llm_config(request: LlmConfigRequest, credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    from api.services.billing_service import classify
    if classify(payload) == "manor" and request.model_fields_set.intersection({
        "stt_api_key", "stt_base_url", "llm_api_key", "llm_model", "llm_base_url",
    }):
        raise HTTPException(403, "Minutes AI is managed by Manor. No separate provider keys are needed in Minutes.")
...
def _initialize_local_auth() -> None:
    """Initialize optional local auth without creating cloud defaults."""
    init_users_table()
    if IS_COMMUNITY:
        seed_default_admin()


# Initialize local users table on import. Cloud uses Manor OAuth and must not
# create a publicly predictable local administrator account.
try:
    _initialize_local_auth()
except Exception as e:
    logger.warning("Could not initialize local users table ({})", type(e).__name__)
