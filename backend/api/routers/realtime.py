...
def _load_user_stt_config() -> dict:
    """Load user's STT API key and base URL from database."""
    try:
        conn = psycopg2.connect(DATABASE_URL, cursor_factory=psycopg2.extras.RealDictCursor)
        with conn.cursor() as cur:
            cur.execute("SELECT stt_api_key, stt_base_url FROM users WHERE stt_api_key IS NOT NULL AND stt_api_key != '' LIMIT 1")
            row = cur.fetchone()
        conn.close()
        if row:
...
async def websocket_transcribe(websocket: WebSocket):
    ...
    if not use_local_stt:
        # Resolve API key and base URL: user's DB config > env var > llm default
        env_key = os.getenv("OPENAI_API_KEY", "").strip()
        env_base_url = os.getenv("OPENAI_BASE_URL") or None
        user_stt_config = _load_user_stt_config()
        ...
        logger.info(
            f"STT config — key: {masked_key} | base_url: {effective_base_url or '(default: api.openai.com)'} | "
            f"source: {'db' if user_stt_config.get('stt_api_key') else 'env'}"
        )
            ...
            if "text" in data:
                try:
                    ...
                    if msg.get("type") == "config":
                        ...
                        # Allow overriding key/base_url from client
