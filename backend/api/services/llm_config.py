...
def resolve_processing_llm_config(*, manor_managed: bool, user_config: Optional[dict] = None) -> dict:
    """Resolve the existing recording pipeline defaults without mutating env.

    Manor accounts use deployment credentials. Local accounts retain their
    own LLM/STT proxy fallback and the pipeline's existing server fallback.
    """
    config = {} if manor_managed else (user_config or {})
    user_key = (config.get("llm_api_key") or config.get("stt_api_key") or "").strip()
    user_base = (config.get("llm_base_url") or config.get("stt_base_url") or "").strip()
    # The recording pipeline historically always used an explicit proxy URL,
    # so its model default is the proxy model even with an OPENAI_API_KEY.
    raw_model = (
        config.get("llm_model") or os.getenv("LLM_MODEL")
        or os.getenv("OPENROUTER_MODEL") or _DEFAULT_MODEL
    ).strip()
    return {
        "api_key": user_key or get_openrouter_api_key(),
        "base_url": user_base or get_openrouter_base_url(),
        "model": _MODEL_ALIASES.get(raw_model, raw_model),
        "use_local": os.getenv("LLM_MODE", "cloud") == "local",
    }
