...
def authenticate_user(email: str, password: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT id, email, password_hash, name, stt_api_key, stt_base_url, llm_api_key, llm_model, llm_base_url, webhook_url FROM users WHERE email = %s", (email.lower().strip(),))
            user = cur.fetchone()
            if not user:
                return None
            ...
            return {"id": str(user["id"]), "email": user["email"], "name": user["name"],
                    "stt_api_key": user.get("stt_api_key"), "stt_base_url": user.get("stt_base_url"),
...
def generate_external_auth_token(
    user_info: Dict[str, Any],
    entity_id: Any,
    auth_source: str,
    access_token: Optional[str] = None,
) -> str:
    """Generate a local JWT for a user authenticated by an external provider."""
    email = (user_info.get("email") or "").strip()
    name = user_info.get("name") or user_info.get("nickName") or user_info.get("userName") or email
    user_id = user_info.get("id") or user_info.get("user_id") or user_info.get("userId") or email
    payload = {
        "sub": str(user_id),
        "email": email,
        "name": name,
    ...
    if access_token:
        payload[f"{auth_source}_token"] = access_token
    return jwt.encode(payload, JWT_SECRET, algorithm=JWT_ALGORITHM)
...
def get_user_by_email(email: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT id, email, name, stt_api_key, stt_base_url, llm_api_key, llm_model, llm_base_url, webhook_url FROM users WHERE email = %s", (email.lower().strip(),))
            user = cur.fetchone()
            return dict(user) if user else None
    except Exception as e:
...
def update_user_llm_config(email: str, stt_api_key: str = None, stt_base_url: str = None, llm_api_key: str = None, llm_model: str = None, llm_base_url: str = None) -> bool:
    ...
    try:
        ...
        if stt_base_url is not None:
            sets.append("stt_base_url = %s")
            vals.append(stt_base_url)
