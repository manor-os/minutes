...
class TranscriptionService:
    ...
    def __init__(self, *, stt_config: Optional[dict] = None):
        ...
        api_key = stt_config["api_key"] if stt_config is not None else os.getenv("OPENAI_API_KEY")
        ...
        # Use OPENAI_BASE_URL if set (user's STT proxy), otherwise default to api.openai.com
        stt_base_url = stt_config["base_url"] if stt_config is not None else os.getenv("OPENAI_BASE_URL") or None
        self.base_url = stt_base_url
        self.client = OpenAI(api_key=api_key, base_url=stt_base_url) if api_key else None
        self.audio_service = AudioService()
        self.model = "whisper-1"

    def _transcription_request_options(self) -> dict:
        ...
        try:
            ...
            with open(audio_file_to_use, "rb") as audio_file:
                transcript = self.client.audio.transcriptions.create(
                    ...
                    file=audio_file,
                    language=language,
                    **self._transcription_request_options(),
                )
        ...
        try:
            ...
            try:
                for i in range(chunk_count):
                    ...
                    chunk_text = getattr(chunk_transcript, "text", "")
                ...
                # Combine all text
                combined_text = " ".join([seg["text"] for seg in all_segments]) or " ".join(all_text)
