...
# HTTP MCP endpoint for the Manor AI platform (cloud edition; opt-in
# elsewhere via MCP_HTTP_ENABLED=true)
MCP_HTTP_ENABLED = IS_CLOUD or os.getenv("MCP_HTTP_ENABLED", "").lower() in ("1", "true")


@contextlib.asynccontextmanager
async def _lifespan(app):
    if MCP_HTTP_ENABLED:
        from api.mcp_http import mcp_lifespan
...
if MCP_HTTP_ENABLED:
    from api.mcp_http import AuthenticatedMCPApp
