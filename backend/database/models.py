...
# JSONB on PostgreSQL, plain JSON elsewhere. A dialect variant resolves per
# engine at compile time, unlike the old DATABASE_URL sniff at import time,
# which broke any non-postgres engine (e.g. sqlite tests) created while a
# postgres URL happened to be in the environment.
def get_json_column_type():
    """Return appropriate JSON column type for database"""
    return JSONB().with_variant(JSON(), "sqlite").with_variant(JSON(), "mysql")
...
class MeetingModel(Base):
    ...
    entity_id = Column(String(64), nullable=False, index=True)  # Entity ID for multi-tenant isolation
    created_by_user_id = Column(String, nullable=True)  # User ID who recorded the meeting (UUID in local auth, numeric in Manor SSO)
    auth_source = Column(String(16), nullable=False, default="local")  # "manor" = Manor SSO OR Google login (both billed via Manor credit); "local" = BYO key. Google logins are normalized to "manor" at stamp time.
    ...
    def to_dict(self, include_user_info=False):
        ...
        result = {
            ...
            "created_by_user_id": str(self.created_by_user_id) if self.created_by_user_id is not None else None,
